...
server <- function(input, output) {
  ...
  observe({
    leafletProxy("map", data = election_results) %>%
      clearMarkers() %>%
      addMarkers(lng = ~longitude, lat = ~latitude,
                 popup = paste("<strong>State:</strong>", election_results$alias, "<br>",
                               "<strong>Party:</strong>", election_results$party, "<br>",
                               "<strong>Votes:</strong>", election_results$vote, "<br>",
                               "<strong>No. of LGAs:</strong>", election_results$noOfLgas, "<br>",
                               "<strong>Registered Voters:</strong>", election_results$registeredVote, "<br>",
                               "<strong>Total Votes:</strong>", election_results$totalVote))
  })
}
